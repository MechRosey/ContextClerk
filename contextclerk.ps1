...
$candidateFiles | ForEach-Object {
    ...
    $logPath = if ($cwd) { Join-Path $cwd 'ContextLog.md' } else { $null }
...
# Phase 4: write ContextLog.md entries in file order (preserves per-project sequence)
foreach ($item in $workItems) {
    ...
    $oldLogPath2 = Join-Path $cwd 'ContextClerk.md'
    if ((Test-Path $oldLogPath2) -and -not (Test-Path $logPath)) {
        Rename-Item $oldLogPath2 $logPath
    }
...
[ContextClerk]($ToolRepo) monitors Claude sessions and updates this file every five minutes when changes occur.
